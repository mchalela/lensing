...
def _map_per_lens(j, dict_per_lens):
    ...
    scaled_dist = dist_hMpc / dL[scale]
    ...
    px = scaled_dist * np.cos(np.deg2rad(theta))
    py = scaled_dist * np.sin(np.deg2rad(theta))
...
class Map(object):
    ...
    def __init__(self, nbins=None, box_size_scaled=None, cosmo=None, back_dz=None):

        self.cosmo = cosmo
        self.back_dz = back_dz
        self.nbins = nbins
        self.box_size_scaled = box_size_scaled
        if box_size_scaled is not None:
            self.bins = gentools.make_bins(-box_size_scaled/2., box_size_scaled/2., nbins=nbins, space='lin') 
...
class ScaledMap(Map):
    ...
    def __init__(self, data_L, data_S, scale, nbins=10, box_size_scaled=0.5, rotate=None,
        cosmo=cosmo, back_dz=0.1, precomputed_distances=True, njobs=1, colnames=None):

        super().__init__(nbins=nbins, box_size_scaled=box_size_scaled, cosmo=cosmo, back_dz=back_dz)
